'''
    ...
    例程使用之前，请确定Line455 opendevice打开的是对应型号的设备
'''
...
def USBCANFD_Start(zcanlib, device_handle, chn):
    ...
    # 使能总线利用率上报
    ret = zcanlib.ZCAN_SetValue(device_handle, "0/set_bus_usage_enable", repr(enable_bus_usage))
    if ret != ZCAN_STATUS_OK:
        print("Set CH%d set_bus_usage_enable failed!" % chn)
        return None

    # 设置总线利用率信息上报周期 20~2000ms
    ret = zcanlib.ZCAN_SetValue(device_handle, "0/set_bus_usage_period", "50".encode("utf-8"))
    if ret != ZCAN_STATUS_OK:
        print("Set CH%d set_bus_usage_enable failed!" % chn)
        return None
...
def Auto_Send_test(device_handle,chn):
    ...
    auto_canfd.interval = 50   # 定时周期，单位ms
    ...
    auto_canfd = ZCANFD_AUTO_TRANSMIT_OBJ()
    memset(addressof(auto_canfd), 0, sizeof(auto_canfd))
    auto_canfd.index = 2  # 定时发送序列号 用于标记这条报文
    auto_canfd.enable = 1  # 使能该条定时发送 0-关闭 1-启用
    auto_canfd.interval = 5000  # 定时周期，单位ms
    auto_canfd.obj.transmit_type = 0  # 0-正常发送，2-自发自收
    auto_canfd.obj.frame.can_id = 0x707  # ID
    auto_canfd.obj.frame.can_id |= 1 << 31
    auto_canfd.obj.frame.len = 64  # 长度
    # auto_canfd.obj.frame.flags |= 0x20  # 发送回显
    for j in range(auto_canfd.obj.frame.len):
...
def Clear_Send_Task(device_handle,chn):
    global auto_send,queue_send    # 声明 标识为 全局变量
    if auto_send:
        ret = zcanlib.ZCAN_SetValue(device_handle, str(chn) + "/clear_auto_send", "0".encode("utf-8"))
        if ret != ZCAN_STATUS_OK:
...
def Queue_Transmit_Test(device_handle,chn,chn_handle):
    ...
    if count < transmit_num:
        ...
        count = zcanlib.Transmit(chn_handle, msgs, transmit_num)
    with print_lock:
        print("队列发送--成功发送 %d 条CAN报文" % count)
    ...
    if count < transmit_num:
        ...
        count = zcanlib.TransmitFD(chn_handle, canfd_msgs, transmit_canfd_num)
    with print_lock:
        print("队列发送--成功发送 %d 条CANFD报文" % count)
...
if __name__ == "__main__":
    ...
    if enable_bus_usage == 1:   #   开启总线利用率上报
        thread = threading.Thread(target=read_bus_usage, args=(handle,0) )   # 开启独立线程获取通道0的总线利用率
        threads.append(thread)
        thread.start()
