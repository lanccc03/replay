'''
    支持的设备有 PCIE-CANFD-100U、PCIE-CANFD-200U-EX （新卡）、PCIE-CANFD-400U、M.2CANFD、MiniPCIeCANFD。
    （PCIE-CANFD-200U 老卡，打开设备要选择 39，且老卡定时发送存在一些问题）
    PCIECANFD系列不需要使能总线利用率上报，即可上报
'''
...
# 启动通道
def PCIECANFD_Start(zcanlib, device_handle, chn):
    ...
    # 设置总线利用率信息上报周期 20~2000ms   --- PCIECANFD系列不需要使能总线利用率上报，即可上报
    ret = zcanlib.ZCAN_SetValue(device_handle, "0/set_bus_usage_period", "50".encode("utf-8"))
    if ret != ZCAN_STATUS_OK:
        print("Set CH%d set_bus_usage_period failed!" % chn)
        return None
...
if __name__ == "__main__":
    ...
    # 打开设备
    handle = zcanlib.OpenDevice(ZCAN_PCIE_CANFD_200U_EX, 0, 0)
    ...
    for i in range(can_number):
        chn_handle = PCIECANFD_Start(zcanlib, handle, i)
        if chn_handle is None:
            print("启动通道%d失败！" % i)
            exit(0)
