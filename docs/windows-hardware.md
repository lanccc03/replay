...
本文说明真实硬件接入前的环境要求、ZLG / 同星（TSMaster）预检查步骤、发送路径注意事项、已知限制与建议联调顺序。
...
- 同星回放发送路径当前应保持异步发送思路，不要默认把 `async` 调用回退成 `sync`
- 如果改成 `sync` / 阻塞式发送，发送线程会在每帧或每批调用上等待，更容易拉低高频或密集回放下的实际发送速率
- 除非有明确的兼容性问题和新的 Windows 真机证据，否则不要把 `async` 发送当成可以随意替换成 `sync` 的等价实现
- 当前关于回放偏差 / 性能问题的已排查结论还包括：切换到 `sync` 发送不行，修改 2ms 切片内间隔也不行；后续分析应默认把这两条视为已排除方向，除非有新的代码证据或 Windows 真机结果推翻它们
